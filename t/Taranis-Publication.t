	...
	for my $type (
		...
		subtest $type->{name_long} => sub {
			withEphemeralDatabase {
				...
				Database->{simple}->update('publication', {status => 3});

				# Test setPublication.
				{
					my $publication_type = Database->{simple}->select(
						'publication_type', 'id', {title => $type->{name_long}}
					)->list;

					Publication->setPublication(id => lastTableId('publication'), status => 2);
					is Database->{simple}->select('publication', ['status'], {id => lastTableId('publication')})->list,
						2,
					...
					cmp_deeply_or_diff [ Database->{simple}->select(
						-from => 'publication',
						-columns => ['status'],
