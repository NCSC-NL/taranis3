...
cmp_select $catch_query::query, <<'__QUERY', 'constructed query';
 SELECT details.id  AS details_id,
        pu.id       AS pub_id,
  ...
  WHERE NOT deleted AND pu.created_on >= '20180109 092311'
    AND (details.id IN (
           ...
           WHERE summary ILIKE ? OR summary ILIKE ?
         ) OR
         details.id IN (
           SELECT d.id
           FROM publication AS pu
                JOIN publication_advisory AS d ON pu.id = d.publication_id
           WHERE contents ILIKE ? OR contents ILIKE ?
         ) OR
