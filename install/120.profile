...
use Taranis::Install::Config qw(config_generic config_release
    sorted_available_versions become_user_taranis appconfig_path);
...
foreach my $v (@versions) {
	...
	my $msg
	  ...
	  :                                 'installed';
	push @menu, sprintf "  %1s %-20s %s", ($is_sel ? '*' : ' '), $version, $msg;
}
...
foreach (@versions) {
	...
	my @bin     = grep -d,
		"$home/bin",                   # manually created bins
		"$home/local-$version/bin",    # local overrule of dist, version spec
		"$home/local/bin",             # local overrule of dist, generic
		"$home/taranis-$version/bin";  # taranis bins
...
if($with_git) {
	...
	my $bin = join ':',
		"$home/bin",               # manually added bins
		"$home/local-git/bin",     # local overrule of dist, version spec
		"$home/local/bin",         # local overrule of dist, generic
		'$DIST/bin',               # bins as appear in dist
		'$DIST/devel',             # helpers during development
		"$home/taranis-git/bin";   # generated bins (like psql wrapper)
