...
INSERT INTO taranis VALUES ('schema_version', 3463);
...
CREATE INDEX email_item_archive_body_trgm_idx ON email_item_archive using GIN(body gin_trgm_ops);
...
-- the advisory_website information (with its generic counterpart) are part
-- of the 'write for website' pluggable interface.  NCSC-NL has multiple
-- implementations for connectors which use this table.
...
CREATE TABLE publication_advisory_website (
    ...
    advisory_forward_id integer,           --    used as kind-of flag
    damage         smallint,
    probability    smallint,
    version        character varying(5),
