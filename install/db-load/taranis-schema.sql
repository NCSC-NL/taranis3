...
--- Slowly, this schema gets transformed from a postgresql dump into a documented
--- schema.

--- The assigned id ranges are listed in
--- https://github.com/NCSC-NL/taranis3/wiki/Primary-key-%28id%29-ranges

--- The schema elements are in the process of being organized better.
...
--- Table "taranis" was added in 3.4 to maintain global settings, mainly: the
...
CREATE TABLE constituent_group (
	...
	external_ref  text,
	PRIMARY KEY (id),
	FOREIGN KEY (constituent_type) REFERENCES constituent_type(id)
);

ALTER SEQUENCE constituent_group_id_seq
	OWNED BY constituent_group.id;

CREATE INDEX ON constituent_group
	USING btree (constituent_type);
...
CREATE TABLE constituent_individual (
    ...
    external_ref text,
	PRIMARY KEY (id),
	FOREIGN KEY (role) REFERENCES constituent_role(id)
);
...
CREATE TABLE constituent_publication (
	...
	FOREIGN KEY (constituent_id) REFERENCES constituent_individual(id),
...
COMMENT ON COLUMN entitlement.particularization
	IS 'This field indicates a particularization for role_right. If true, the role_right (web)form should display a text field.';
...
--- items are addressed by digest.  This should move to use the id
CREATE TABLE item (
...
CREATE TABLE publication_advisory_website (
	...
	FOREIGN KEY (advisory_forward_id) REFERENCES publication_advisory_forward(id)
);
...
ALTER SEQUENCE publication_advisory_website_id_seq
	OWNED BY publication_advisory_website.id;
...
CREATE TABLE calling_list (
	...
	FOREIGN KEY (group_id)       REFERENCES constituent_group(id),
	FOREIGN KEY (publication_id) REFERENCES publication(id),
	FOREIGN KEY (locked_by)      REFERENCES users(username)
);
...
ALTER TABLE advisory_damage
	ADD CONSTRAINT damage_description_damage_id_fkey
		FOREIGN KEY (damage_id) REFERENCES damage_description(id);
