...
sub collect_source {
	...
	ITEM:
		foreach my $item (@feed) {
			...
			#XXX Fight parallel feed insert race condition.  With parallel
			# collector threads, there may be other processes which found
			# exactly the same text on other website at the same time.  This,
			# for instance, happens often with GoogleNews.
			$stmnt .= ' ON CONFLICT DO NOTHING';
			...
			trace "CheckId, getting SourceData $link" if $debugSource;
			...
			$collector->writeCVEs($digest, @certids);
		}
	}
