...
function setOwnershipCallback ( params ) {
	...
	if ( params.message ) {
		...
		var ownershipButton = $('i[data-analysisidownership="' + params.analysisId + '"]');
		if ( ownershipButton.hasClass('analyze-item-has-no-owner') || ownershipButton.hasClass('analyze-item-is-not-owner') ) {
			ownershipButton
				.attr({ 'title': 'you are owner' })
				.removeClass('analyze-item-has-no-owner analyze-item-is-not-owner far')
				.addClass('analyze-item-is-owner fas');
		} else {
			ownershipButton
				.attr({ 'title': 'has no owner' })
				.removeClass('analyze-item-is-owner fas')
				.addClass('analyze-item-has-no-owner far');
		}		
