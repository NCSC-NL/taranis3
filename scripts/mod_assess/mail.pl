	...
	if ( right("execute") ) {
		...
		foreach my $unescapedId ( @ids ) {
			...
			if ( $item->{is_mail} ) {
				...
				$item->{description} = trim( HTML::Entities::encode( decodeMimeEntity( $entity, 1, 1 ) ) );
			}
...
sub mailItem {
	...
	if ( right("execute") ) {
		...
		my @addresses = ref $kvArgs{mailto} eq 'ARRAY' ? @{ $kvArgs{mailto} } : $kvArgs{mailto};

		my $subject   = HTML::Entities::decode $kvArgs{subject};
		$subject      =~ s/\s+/ /g;

		my $from_name = HTML::Entities::decode $user_settings->{mailfrom_sender};
...
sub mailMultipleItems {
	...
	if ( right("execute") ) {
		...
		foreach my $mailItem ( @$mailItems ) {
			...
			my $subject   = HTML::Entities::decode( $mailItem->{subject} );
			$subject      =~ s/\s+/ /g;
