...
sub addPendingItem {
	...
	if (
		...
		my @dossiers = flat $kvArgs{dossier};
		...
		foreach my $dossierID ( @dossiers ) {
			...
			if ( $kvArgs{type} =~ /^(assess|analyze)$/ ) {
				...
				my @assessItems = $kvArgs{type} eq 'assess'
					? $oTaranisAssess->getItem( $kvArgs{itemid} )
					: @{ $oTaranisAnalyze->getLinkedItems( $kvArgs{itemid} ) };

				foreach my $assessItem ( @assessItems ) {
					next if $assessItem->{screenshot_object_id}; # already
					next if $assessItem->{is_mail};

					my $link    = $assessItem->{link} or next;
					my $picture = takeScreenshotOfAssessItem($link);
					$picture or next;

					my %assessUpdate = ( digest => $assessItem->{digest} );

					if(my $blobDetails = $oTaranisDossierItem->{dbh}->addFileAsBlob( binary => $picture->{binary})) {

						$assessUpdate{screenshot_file_size} = $blobDetails->{fileSize};
						$assessUpdate{screenshot_object_id} = $blobDetails->{oid};
						$oTaranisAssess->setAssessItem(%assessUpdate)
							or $message = $oTaranisAssess->{errmsg};
					}
				}
			}
...
sub takeScreenshotOfAssessItem {
	...
	my %opts = ( screenshot_module => Config->{screenshot_module} ); 
	$opts{proxy_host} = Config->{proxy_host} if Config->{proxy_host};
	$opts{user_agent} = Config->{useragent}  if Config->{useragent};

	my $shooter = Taranis::Screenshot->new(%opts);
	my $screenshot = $shooter->takeScreenshot(siteAddress => $url)
		or return;
