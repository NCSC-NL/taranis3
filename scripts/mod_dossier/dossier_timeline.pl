...
sub openDialogDossierMailItem {
	my %kvArgs = @_;
	my $itemId   = val_int $kvArgs{itemid};
	my $itemType = val_string $kvArgs{itemtype};
	...
	if($itemId && $itemType && $contentTypes->{$itemType}) {
		...
		if($constituentIndividualRight) {
			my $ci = Taranis::Constituent::Individual->new(Config);
			my @individuals = $ci->loadIndividuals;
			$ci->mergeGroupNames($_) for @individuals;
			$vars->{constituentIndividuals} = \@individuals;
		}
		
		my $dossierItem = $oTaranisDossierItem->getDossierItems(id => $itemId)->[0];
		...
		$vars->{dossierItemContent} = $oTaranisDossierItem->getDossierItemContent( contentType => $itemType, dossierItem => $dossierItem );
		$vars->{dossierItem} = $dossierItem;
		$vars->{dossierItemType} = $itemType;
...
sub mailDossierItem {
	...
	if ( right("execute") ) {
		...
		foreach my $key ( 'dossier_attachment', 'dossier_attachment_note' ) {
			foreach my $attachmentID (flat $kvArgs{$key}) {
				$attachmentID =~ s/&quot;/"/g;
