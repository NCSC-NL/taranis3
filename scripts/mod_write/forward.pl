...
sub openDialogNewForward {
	...
	my $typeName = Taranis::Config->new( Config->{publication_templates} )->{advisory}->{forward};
	...
	if ( rightOnParticularization( $typeName ) && right('write') ) {
		...
		if ( $analysisId ) {
			...
			if ( @platformIds ) {
				...
				( $vars->{platforms_text}, undef ) = Publication->processPreviewTemplatePart( to_json( $pre_json_obj ), "software_hardware" );
			}
	
			if ( @productIds ) {
				$pre_json_obj = { ids => \@productIds, columns => ['producer', 'name' ] };
				( $vars->{products_text}, undef ) = Publication->processPreviewTemplatePart( to_json( $pre_json_obj ), "software_hardware" );
	
				$pre_json_obj = { ids => \@productIds, columns => ['version'] };
				( $vars->{versions_text}, undef ) = Publication->processPreviewTemplatePart( to_json( $pre_json_obj ), "software_hardware" );
			} 
		}	
...
sub openDialogUpdateForward {
	...
	if ( rightOnParticularization( $typeName ) && $writeRight ) {
		...
		if ( @newCveIds ) {
			...
			if ( @platformIds ) {
				$pre_json_obj = { ids => \@platformIds, columns => ['producer', 'name', 'version' ] }; 
				$vars->{platforms_text} .= "\n" . Publication->processPreviewTemplatePart( to_json( $pre_json_obj ), "software_hardware" );
			}
...
sub saveNewForward {
	...
	if ( rightOnParticularization( $typeName ) && right('write') ) {
		...
		my $typeId = Publication->getPublicationTypeId( $typeName )->{id};
		my $userId = sessionGet('userid');
		...
		if ( $kvArgs{screenshot_url} ) {
			@screenshotURLs = ( ref( $kvArgs{screenshot_url} ) =~ /^ARRAY$/ )
				? @{ $kvArgs{screenshot_url} }
				: $kvArgs{screenshot_url};
			@screenshotDescriptions = ( ref( $kvArgs{screenshot_description} ) =~ /^ARRAY$/ )
				? @{ $kvArgs{screenshot_description} }
				: $kvArgs{screenshot_description};
		}
		...
		withTransaction {
			...
			if ( $kvArgs{damage_description} ) {
				...
				foreach my $damageDescriptionID ( @damageDescriptions ) {
					Publication->linkToPublication(
						table => 'advisory_forward_damage',
...
sub saveForwardDetails {
	...
	if ( rightOnParticularization( $typeName ) && right('write') ) {
		...
		if ( $kvArgs{publication_attachment} ) {
			...
			my @submittedAttachments = ( ref( $kvArgs{publication_attachment} ) =~ /^ARRAY$/ )
				? @{ $kvArgs{publication_attachment} }
				: $kvArgs{publication_attachment};
