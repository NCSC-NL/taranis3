...
sub openDialogNewAdvisory {
	...
	my $typeName = Taranis::Config->new( Config->{publication_templates} )->{advisory}->{email};
	...
	if ( rightOnParticularization( $typeName ) && right('write') ) {
		...
		if ( $analysisId ) {
			...
			if ( @platformIds ) {
				...
				( $vars->{platforms_text}, undef ) = $oTaranisPublication->processPreviewTemplatePart( to_json( $pre_json_obj ), "software_hardware" );
			}

			if ( @productIds ) {
				$pre_json_obj = { ids => \@productIds, columns => ['producer', 'name' ] };
				( $vars->{products_text}, undef ) = $oTaranisPublication->processPreviewTemplatePart( to_json( $pre_json_obj ), "software_hardware" );

				$pre_json_obj = { ids => \@productIds, columns => ['version'] };
				( $vars->{versions_text}, undef ) = $oTaranisPublication->processPreviewTemplatePart( to_json( $pre_json_obj ), "software_hardware" );
			}
		}
...
#		}
		...
		if ( @newCveIds ) {
			...
			if ( @platformIds ) {
				$pre_json_obj = { ids => \@platformIds, columns => ['producer', 'name', 'version' ] };
				$vars->{platforms_text} .= "\n" . $oTaranisPublication->processPreviewTemplatePart( to_json( $pre_json_obj ), "software_hardware" );
			}
		...
		if ( $kvArgs{analysisId} =~ /^\d+$/i ) {
			@itemLinksNewAnalysis = @{ $oTaranisPublication->getItemLinks( analysis_id => $kvArgs{analysisId} ) };
		}
...
sub openDialogImportAdvisory {
	...
	if ( rightOnParticularization( $typeName ) && right('write') ) {
		if (
			...
			my $emailItem = $oTaranisAssess->getMailItem( $kvArgs{emailItemId} );
			...
			if ( $@ ) {
				...
				my $attachments = $oTaranisAssess->getAttachmentInfo( $messageEntity, undef );
...
sub openDialogPreviewAdvisory {
	...
	if ( rightOnParticularization( $typeName ) ) {
		...
		$vars->{created_by_name} = ( $advisory->{created_by} ) ? $oTaranisUsers->getUser( $advisory->{created_by}, 1 )->{fullname} : undef;
		$vars->{approved_by_name} = ( $advisory->{approved_by} ) ? $oTaranisUsers->getUser( $advisory->{approved_by}, 1 )->{fullname} : undef;
		$vars->{published_by_name} = ( $advisory->{published_by} ) ? $oTaranisUsers->getUser( $advisory->{published_by}, 1 )->{fullname} : undef;
		...
		withTransaction {
			if (
				...
				if ( $kvArgs{pf_left_column} ) {
					...
					foreach my $platformId ( @platforms ) {
						if (
							!$oTaranisPublication->linkToPublication(
				...
				if ( $kvArgs{damage_description} ) {
					...
					foreach my $damageDescription ( @damageDescriptions ) {
						if (
							!$oTaranisPublication->linkToPublication(
								table => 'advisory_damage',
			...
			if ( exists( $kvArgs{publication_previous_version_id} ) && $kvArgs{publication_previous_version_id} ) {
				if ( $kvArgs{publication_previous_version_id} && !$oTaranisPublication->setPublication( id => $kvArgs{publication_previous_version_id}, replacedby_id => $publicationId ) ) {
					$message = $oTaranisPublication->{errmsg};
				}
			}
		};
