...
sub openDialogNewEod {
	...
	if ( rightOnParticularization( $typeName ) ) {
		...
		my @users;
		while(my $user = $oTaranisUsers->nextObject) {
			push @users, $user;
		}  
		
		my $begin = PublicationEndOfDay->getLatestTimeframeEndOfPublishedEOD
		  || nowstring(13) . ' 15:00:00';
		my $time  = $begin =~ m/\s(\d\d:\d\d:\d\d)$/a ? $1 : '15:00:00';
		my $end   = nowstring(11) . ' ' . $time;
		$vars->{eod}   = { timeframe_begin => $begin, timeframe_end => $end };
		$vars->{users} = \@users;
...
sub _date_time_pubtype($) {
	...
	my $beginDate = formatDateTimeString $args->{begin_date};
	my $endDate   = formatDateTimeString $args->{end_date};
	my $beginTime = $args->{begin_time};
	my $endTime   = $args->{end_time};
	my $pubtype   = $args->{publicationTypeId};

	$beginDate && $endDate
		or $message = "Invalid date format supplied. Please specify a date by 'dd-mm-yyyy'.";
