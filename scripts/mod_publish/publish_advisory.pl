...
sub publishAdvisory {
	...
	if ( $hasPublicationRights ) {
		...
		my $typeName = Taranis::Config->new( Config->{publication_templates} )->{advisory}->{email};
		my $typeIdAdvisoryEmail = Publication->getPublicationTypeId( $typeName )->{id};

		$typeName = Taranis::Config->new( Config->{publication_templates} )->{advisory}->{taranis_xml};
		my $typeIdAdvisoryXml = Publication->getPublicationTypeId( $typeName )->{id};

		my $isHighHigh = 0;
		if (scalar( @groups ) eq 0) {
