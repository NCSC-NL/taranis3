...
use Taranis::FunctionalWrapper qw(CGI Config Database Publication Publish Template);
...
sub publishAdvisory {
	...
	if ( $hasPublicationRights ) {
		...
		if (@groups == 0) {
			...
			my @emailOnly = grep defined, @wantEmail;

			my $title   = $publication->{title};
			my $certid  = $publication->{govcertid};
			my $version = $publication->{version};
			my $damage  = $publication->{damage};
			my $probab  = $publication->{probability};

			$isHighHigh = $damage==1 && $probab==1;

			my %level = ( 1 => "H", 2 => "M", 3 => "L" );

			my $subject = "$certid [v$version] [$level{$probab}/$level{$damage}] " .
			...
			if ( @addresses > 0 ) {
				...
				$sendingFailed = 1 if $response ne "OK";
			...
			if(! $sendingFailed) {
				...
				my $updatedAnalysisCount = Publish->setAnalysisToDoneStatus( $publicationId, $certid);

				$vars->{analysisUpdated} = $updatedAnalysisCount;

				# in case of a High/High advisory create a callinglist
				if ( $damage == 1 && $probab == 1 ) {
			...
			if(Config->publishWebsite) {
				withTransaction {
					Publication->addPublication(
						title      => $title,
						created_by => sessionGet('userid'),
						created_on => nowstring(10),
						type => Publication->getPublicationTypeId(advisory=> 'website'),
						status     => 2,
					);
					my $publicationIdWebsite = Database->getLastInsertedId("publication");
					Publication->linkToPublication(
						table => "publication_advisory_website",
						publication_id => $publicationIdWebsite,
						version     => $version,
						govcertid   => $certid,
						damage      => $damage,
						probability => $probab,
				...
				$vars->{results} = "A new Advisory Website publication has been created.\n$vars->{results}";
			}

				# update all advisories with same based_on ID
				if($publication->{based_on} =~ /(.*?) \d\.\d\d$/a) {
					my $basedOnId = $1;
			...
			setUserAction( action => 'publish advisory', comment => "Published advisory $certid [v$version]");
		}
