...
sub displayFeedDigests {
	...
	my $htmlContent = $tt->processTemplate('feed_digest_overview.tt', $vars, 1);
	my $htmlFilters = $tt->processTemplate('feed_digest_filters.tt', $vars, 1);
...
sub openDialogNewFeedDigest {
	...
	my $tt = Taranis::Template->new;
	my $dialogContent = $tt->processTemplate('feed_digest_details.tt', $vars, 1);
...
sub addFeedDigest {
	...
	if( !$url || !$url->scheme) {
		...
		if(my $feedID = Database->addObject(feeddigest => \%insert, 1) ) {
			...
			my $tt = Taranis::Template->new;
			$itemHtml = $tt->processTemplate('feed_digest_item.tt', $vars, 1 );
			$addOk = 1;
		} else {
	...
	if ( $addOk ) {
		setUserAction( action => 'add feed digest', comment => "Added feed $url");
	} else {
		setUserAction( action => 'add feed digest', comment => "Got error $message while trying to add feed $url");
	}
...
sub saveFeedDigest {
	...
	if($saveOk) {
		setUserAction( action => 'edit feed digest', comment => "Edited feed $url");
	} else {
		setUserAction( action => 'edit feed digest', comment => "Got error $message while trying to edit feed $url");
	}	
