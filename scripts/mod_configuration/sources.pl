	...
	if ( right("write") ) {
		...
		if(!$message) {
			...
			if (
				...
				my @wordlist1 = ref $kvArgs{wordlist1} eq 'ARRAY'
					? @{ $kvArgs{wordlist1} }
					: $kvArgs{wordlist1};

				my @wordlist2 = ref $kvArgs{wordlist2} eq 'ARRAY'
					? @{ $kvArgs{wordlist2} }
					: $kvArgs{wordlist2};
	...
	if ( right("write") && $kvArgs{id} =~ /^\d+$/ ) {
		...
		if ( $proceed ) {
			...
			if ( $orig_digest eq $new_digest || !$oTaranisSources->{dbh}->checkIfExists( \%where, 'sources' ) ) {
				...
				my @currentWordlistsID = ref $kvArgs{source_wordlist_id} eq 'ARRAY'
					? @{ $kvArgs{source_wordlist_id} }
					: $kvArgs{source_wordlist_id};
				...
				if (
					...
					my %wordlistsCheck;
					my @wordlist1 = ( ref( $kvArgs{wordlist1} ) =~ /^ARRAY$/ )
						? @{ $kvArgs{wordlist1} }
						: $kvArgs{wordlist1};

					my @wordlist2 = ( ref( $kvArgs{wordlist2} ) =~ /^ARRAY$/ )
						? @{ $kvArgs{wordlist2} }
						: $kvArgs{wordlist2};
	...
	if ( $kvArgs{collector_module} && $kvArgs{collector_module} =~ /^Taranis::Collector::/ ) {
		...
		if ( exists( $collectorModules->{ $kvArgs{collector_module} } ) ) {
			...
			if ( $collector->can('testCollector') ) {
				...
				if ( ref( $configKeys ) =~ /^ARRAY$/ ) {

					foreach my $configKey ( @$configKeys ) {
						$source{ $configKey } = ( exists( $kvArgs{$configKey} ) )
							? decode_entities( $kvArgs{$configKey} )
							: '';
					}
				}
