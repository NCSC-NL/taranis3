...
sub saveSourceDetails {
	my %kvArgs = @_;
	my ($message, $sourceName);
	my $saveOk = 0;

	#XXX Mildly cleaned-up code.  Changes should be made to above
	#XXX saveNewSource as well.

	my $id     = val_int $kvArgs{id};
	if(right("write") && $id) {
		my $oTaranisSources = Taranis::Sources->new( Config );

		#XXX URL processing should use URI module
		my $url      = $kvArgs{url};
		$url = '/' . $url if $url !~ m#^/#; # ensure leading slash

		my $protocol = lc $kvArgs{protocol};
		my $host     = $kvArgs{host};
		my $port     = val_int $kvArgs{port};
		...
		my $additionalConfigJSON;
		if(my $module = $kvArgs{collector_module} ) {
			my $collectorModules = getCustomCollectorModules();
			my $configKeys = $collectorModules->{$module};

			my %additionalConfig;
			if ( ref $configKeys eq 'ARRAY') {
				$additionalConfig{collector_module} = $module;

				$additionalConfig{$_} = $kvArgs{$_} // ''
					for @$configKeys;
			}
			$additionalConfigJSON = to_json( \%additionalConfig );
			$useForDigest .= $additionalConfigJSON;
		...
		my $category   = $kvArgs{category};
		my $new_digest = textDigest($host . $useForDigest);

		# not all digests have a category indicator
		$new_digest .= ";$category" if $orig_digest =~ /\;/;

		my $addSlashes = $protocol =~ /^imap|^pop3/ ? '://' : '';
		my $full_url   = $protocol . $addSlashes . $host;
		...
		my $rating = val_int $kvArgs{rating} || 50;
		$rating = 50 if $rating < 0 || $rating > 100;

		my $proceed = 1;
		if ( $kvArgs{'source-icon'}==1 ) {
			$sourceName  = sanitizeInput(filename => $kvArgs{new_sourcename});
			my $filename = scalarParam("new_icon");
			my $fh   = CGI->upload( $filename );
			my $mime = CGI->upload_info( $filename, 'mime' );
			my $size = CGI->upload_info( $filename, 'size' );
			...
			$sourceName  = sanitizeInput(filename => $kvArgs{sourcename});
		}
...
sub getCustomCollectorModules {
	...
	my $modules = scan_for_plugins 'Collector';
