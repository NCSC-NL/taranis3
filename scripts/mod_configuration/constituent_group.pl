...
sub openDialogConstituentGroupDetails {
	...
	if ( $kvArgs{id} =~ /^\d+$/ ) {
		...
		if ( @memberIDs ) {
			...
			$vars->{all_individuals} = [ values %individualsNonMemberHASH ];
		} else {
			$vars->{all_individuals} = \@allConstituentIndividuals;
		}
	...
	if ( right("write") ) {
		if ( !$cg->{dbh}->checkIfExists( { name => $kvArgs{name}, status => { "!=", 1 } }, "constituent_group",	"IGNORE_CASE" ) ) {
			withTransaction {
				...
				foreach my $newMember (flat $kvArgs{members}) {
					$cg->addObject(
						table => "membership",
						group_id => $groupId,
						constituent_id => $newMember,
					);
				...
				foreach my $newSWHW (flat $kvArgs{sh_left_column}) {
					$cg->addObject(
						table => "soft_hard_usage",
						group_id => $groupId,
						soft_hard_id => $newSWHW,
					);
...
sub saveConstituentGroupDetails {
	...
	if ( right("write") && $kvArgs{id} =~ /^\d+$/ ) {
		...
		foreach my $id (flat $kvArgs{members}) {
		...
		foreach my $id (flat $kvArgs{sh_left_column}) {
