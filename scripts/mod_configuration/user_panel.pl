...
sub openDialogUserSettings {
	...
	my @allowedCategorieIds = map $_->{id}, @$assessCategoriesFromSession;
	...
	foreach my $search ( @$searches ) {
		my @uriw = split //, $search->{uriw};
		$search->{checku} = 1 if $uriw[0] eq '1';
		$search->{checkr} = 1 if $uriw[1] eq '1';
		$search->{checki} = 1 if $uriw[2] eq '1';
		$search->{checkw} = 1 if $uriw[3] eq '1';
	}
