...
sub openDialogConstituentIndividualDetails {
	...
	if ( $kvArgs{id} =~ /^\d+$/ ) {
		...
		if ( @groupIDs ) {
			...
			$vars->{all_groups} = [ values %groupsNonMemberHASH ];
		} else {
			$vars->{all_groups} = \@constituentGroups;
		}
		...
		if ( @typeIDs ) {
			...
			$vars->{all_types} = [ values %typesNonSelectedHASH ];
		} else {
	...
	if ( right("write") ) {
		...
		if ( !$oTaranisConstituentIndividual->{dbh}->checkIfExists({ 
			...
			withTransaction {
				...
				foreach my $newGroup (flat $kvArgs{membership_groups}) {
					$oTaranisConstituentIndividual->addObject( table => "membership", group_id => $newGroup, constituent_id => $individualId);
				...
				foreach my $newType (flat $kvArgs{selected_types}) {
					$oTaranisConstituentIndividual->addObject( table => "constituent_publication", type_id => $newType, constituent_id => $individualId	);
					$message = $oTaranisConstituentIndividual->{errmsg} if ( $message !~ /$oTaranisConstituentIndividual->{errmsg}/g && $oTaranisConstituentIndividual->{errmsg} ne "" );
				}
			};
		} else {
...
sub saveConstituentIndividualDetails {
	...
	if ( right("write") && $kvArgs{id} =~ /^\d+$/ ) {
		...
		foreach my $id (flat $kvArgs{membership_groups}) {
		...
		foreach my $id (flat $kvArgs{selected_types}) {
