...
sub openDialogPhotoIssueDetails {
	...
	my $issueNr = val_int $kvArgs{id};
	if ($issueNr) {
		my $sh = Taranis::SoftwareHardware->new( Config );
		my $ip = Taranis::ImportPhoto->new( Config );
		$issue = $ip->getIssueDetails($issueNr);

		if ( !$executeRight && $issue->{status} != 0 ) {
			die 403;
		}

		my $issueType = $issue->{issuetype};
		if($issueType == 2) {
			my ($duplicates, $usage) = $ip->getDuplicates(
				producer => $issue->{producer},
				product  => $issue->{name},
				type     => $issue->{type},
			);

			$vars->{duplicates} = $duplicates;
			$vars->{totalUsageCount} = $usage;

			$_->{constituentGroups} = $sh->getConstituentUsage($_->{id})
				for @$duplicates;

		} elsif ($issueType == 3 && $issue->{soft_hard_id} ) {
			$vars->{selectedSH} = $sh->getList(id => $issue->{soft_hard_id});
		}

		my $father = $issue->{followup_on_issue_nr};
		if($issueType != 4) {
			my $lookUpIssue = $father || $issueNr;
			$vars->{photoList} = $ip->getPhotosForIssue( 'ish.issue_nr' => $lookUpIssue );
		}

		# issue followup depth is max 2
		if(($issueType==1 || $issueType==4) && $father) {
			my $parents = $issue->{followupIssues} = [];

			my $top = $ip->getIssueDetails($father);
			push @$parents, $top;

			if(my $granddad = $top->{followup_on_issue_nr}) {
				$top = $ip->getIssueDetails($granddad);
				push @$parents, $top;
			}
			$vars->{selectedSH} = $sh->getList(id => $top->{soft_hard_id});
		}
...
sub getIssueItemHtml {
	...
	my ($vars, $tpl, $status);
	my $ip = Taranis::ImportPhoto->new( Config );
	if(my $issue  = $ip->getIssueDetails($issueNr)) {
		$status                = lc $statusDictionary{$issue->{status}};
		$vars->{issue}         = $issue;
		$vars->{write_right}   = right("write");
		$vars->{execute_right} = right("execute");
...
sub issueReadyForReview {
	my %kvArgs  = @_;
	my $issueNr = val_int $kvArgs{issueNr};
	my $message;
	...
	if ( $kvArgs{issueNr} =~ /^\d+$/ && right("execute") ) {
		...
		withTransaction {
			...
			foreach my $group ( @$groups ) {
				my $shType = $softwareHardwareTypes->{$softwareHardware->{type}} || '';
		...
		setUserAction( action => 'resolve photo issue',
			comment => "Resolved issue '#$issueNr'");
	} else {
		$message = 'No permission.';
		setUserAction( action => 'resolve photo issue',
			comment => "Got error '$message' while trying resolve issue '#$issueNr'");
	}
...
sub dontImport {
	...
	if ( $kvArgs{photo_id} =~ /^\d+$/ && $kvArgs{sh_id} =~ /^\d+$/ && right("execute") ) {
		...
		my $issueComments = "Constituent: " . $photoDetails->{name}
			. "\nVendor: " . $softwareHardware->{producer}
...
sub openDialogImportPhoto {
	...
	if ( right("execute") ) {
		...
		my $cg = Taranis::Constituent::Group->new( Config );
		my @constituentGroups = map +{ name => $_->{name}, id => $_->{id} },
			$cg->searchGroups;
...
sub loadImportFile {
	my %kvArgs = @_;
	my ($message, $tpl, $vars, $photoId);

	my $writeRight = right("write");
	my $constituentId = val_int $kvArgs{constituentGroup};
	my $separator = $kvArgs{separator} =~ /^.$/ ? $kvArgs{separator} : ',';
	my $filename = scalarParam("csv_file");

	if ( right("execute") && defined $filename && $constituentId ) {

		if (my $fh = CGI->upload($filename)) {
			...
			$ip->addImportPhoto(group_id => $constituentId);
			$photoId = $dbh->getLastInsertedId( 'import_photo' );
		...
		CSVENTRY:
			while (<$fh>) {
				...
				@fields or next CSVENTRY;    # silently ignore empty lines

				if(@fields < 4) {
					$message .= "Too few fields in line $linenr\n";
					next;
				}

				next CSVENTRY                # skip optional header
					if $linenr==1 && $fields[0] eq 'Vendor';

				my %record;
				@record{ qw/producer name cpe_id type/ } =
					map encode_entities(trim($_)), @fields;

				my $sh_id;
				my $has = $ip->getImportSoftwareHardware(%record)->[0];
				if($has) {
					$sh_id = $has->{id};
				} else {
					$ip->addImportSoftwareHardware(%record);
					$sh_id = $dbh->getLastInsertedId('import_software_hardware');
				}
			...
			if(keys %sh_ids) {
				...
				$dbh->{db_error_msg} = 'No photo imports, do rollback!';
...
sub getPhotoDetails {
	...
	my $photoId = val_int $kvArgs{id};
	if ( right("execute") && $photoId ) {
		my $ip = Taranis::ImportPhoto->new( Config );
...
sub exportAllPhotos {
	...
	my $csv = Taranis::CsvBuilder->new( quote_all => 0 );
	$csv->addLine( "Vendor", "Product", "CPE", "Type", "Constituent" );
