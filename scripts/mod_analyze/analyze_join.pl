...
sub joinAnalysis {
			...
			if ( $an->addObject( table => 'analysis', title => $title, status => lc( $kvArgs{status} ) ) ) {
				...
				foreach my $id ( @ids ) {
					if ( length( $id ) == 8 ) {
						...
						my $analysisName = analysis_name $id;
						$newAnalysisComments .= "\n[=== Comments from joined analysis $analysisName ===]\n$analysis->{comments}\n";

						$idstring .= $analysis->{idstring};

						$newAnalysisRating = $analysis->{rating} if ( $analysis->{rating} > $newAnalysisRating && $analysis->{rating} ne 4 );

						my $existingAnalysisComments = "[=== WARNING: this analysis was joined into $analysisName ===]\n\n$analysis->{comments}";
	...
	my $joinedAnlyses = join ', ', map analysis_name($_), @ids;

	if ( $analysisJoined ) {
		setUserAction( action => "join analysis", comment => "Joined analyses $joinedAnlyses to new analysis ". analysis_name $newAnalysisId);
	} else {
