...
my $oTaranisConfig = Taranis::Config->new();
my $oTaranisAnalysis = Taranis::Analysis->new( $oTaranisConfig );
my $oTaranisPublication = Taranis::Publication->new( $oTaranisConfig );
my $settings;
my $advisoryPrefix = $oTaranisConfig->{advisory_prefix};
my $advisoryIdLength = $oTaranisConfig->{advisory_id_length};
...
sub setOwnership {
	...
	my $action = ( $username ) ? 'taken' : 'released';
	if ( $oTaranisAnalysis->setOwnerShip( $username, $analysisId ) ) {
		$ownershipSet = 1;
		setUserAction( action => substr( $action, 0, -1 ) . " analysis ownership", comment => ucfirst( $action ) . " ownership of analysis AN-" . substr( $analysisId, 0, 4 ) . '-' . substr( $analysisId, 4, 4) );
	}  else {
		$message = $oTaranisAnalysis->{errmsg};
		setUserAction( action => substr( $action, 0, -1 ) . " analysis ownership", comment => "Got error '$message' while trying to " . substr( $action, 0, -1 ) . " ownership of analysis AN-" . substr( $analysisId, 4 ) . '-' . substr( $analysisId, 3, 4) );
	}
...
sub unlinkItem {
	...
	if ( 
		...
		if ( $oTaranisAnalysis->unlinkItem( $item->{digest}, $analysisId ) ) {
			...
			$isUnlinked = 1;
			setUserAction( action => "unlinked item from analysis", comment => "unlinked '$item->{title}' from AN-" . substr( $analysisId, 0, 4 ) . '-' . substr( $analysisId, 4, 4) );
		}  else {
			$message = $oTaranisAnalysis->{errmsg};
			setUserAction( action => "unlinked item from analysis", comment => "Got error '$message' while trying to unlinked '$item->{title}' from AN-" . substr( $analysisId, 0, 4 ) . '-' . substr( $analysisId, 4, 4) );
		}
	}
...
sub formatAnalysisRecord {
	...
	ADVISORYID:
	foreach my $id ( $comments =~ /($advisoryPrefix-\d{4}-\d{$advisoryIdLength})/gi ) {
		if ( exists( $foundAdvisoryIds{ $id } ) ) {
			next ADVISORYID;
		}
		$foundAdvisoryIds{ $id } = 1;
		my $advisory = $oTaranisPublication->getLatestAdvisoryVersion( govcertId => uc( $id ) );

		my ( $publicationType, $advisoryTable );
		
		if ( exists( $advisory->{source} ) ) {
			$publicationType = 'forward';
			$advisoryTable = 'publication_advisory_forward';
		} else {
			$publicationType = 'advisory';
			$advisoryTable = 'publication_advisory';
		}
		
		$comments =~ s/$id/$id\[$advisory->{publication_id}][$publicationType]/gi if ( $advisory );
		
		my $advisories = $oTaranisPublication->getPublishedPublicationsByAnalysisId( table => $advisoryTable, govcertid => uc( $id ), analysis_id => $record->{id} );
