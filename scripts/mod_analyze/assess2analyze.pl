...
#TODO: translate below		
	...
	#### alle gevonden analyses worden in selectie veld getoont
			...
			$vars->{analysis_id_match} = $an->getRelatedAnalysisIdMatch( $analysis_rights, $analysis_ids, @collection);
	...
	############## KEYWORD MATCHING ################
			...
			$vars->{analysis_keyword_match} = $an->getRelatedAnalysisKeywordMatch( $analysis_rights, $analysis_ids, @keywords );
...
sub createAssessAnalysis {
	...
	if ( right("write") && rightOnParticularization( $item_category ) ) {
		...
		my $idstring = join ' ', $kvArgs{ids} =~ /(CVE-\d{4}-\d{4,10})/g;
		...
		withTransaction {
			if ( !$an->addObject( 
				...
				if ( exists( $kvArgs{'include-clustered-items'} ) ) {
					...
					my @clusterItemIds = flat $kvArgs{clusterItemId};
	...
	if ( $analysisCreated ) {
		setUserAction( action => "create analysis", comment => "Created analysis ". analysis_name $analysis_id);
	} else {
...
sub linkAssessAnalysis {
	...
	my $analysisName = analysis_name $linkToAnalysis;
	if ( $analysisLinked ) {
		setUserAction( action => "link to analysis", comment => "Linked '$item->{title}' to analysis $analysisName");
	} else {
		setUserAction( action => "link to analysis", comment => "Got error '$message' while trying to link '$item->{title}' to analysis $analysisName");
	}
...
sub searchAssessAnalysis {
	...
	$_->{name} = analysis_name($_->{id}) for @$analysis;
