...
sub withEphemeralDatabase (&) {
	...
	withRollback {
		if ($ephemeralDatabase) {
			my $version_string = Database->{simple}->query('select version()')->list;
			my ($version) = ($version_string =~ /^PostgreSQL (\d\d?\.\d\d?)/)
				or die "Can't determine PostgreSQL version from string: $version_string";

			if ($version >= 9.1) {
				Database->do('create extension lo');
			} else {
				# Old school PostgreSQL, before 'create extension' was invented. We don't really supported these
				# versions anymore, so maybe remove this one day.
				Database->do(q{
			...
			Database->do(fileToString(dirname(__FILE__) . "/../../database_alterations/rss-schema.sql"));
			Database->do(fileToString(dirname(__FILE__) . "/../../database_alterations/Taranis_initiele_inserts.sql"));
		}
	...
	if ($ephemeralDatabase) {
		$clientPids = (
			...
			eval { Database->{simple}->query("select pid from pg_stat_activity") } ||
			# PostgreSQL 9.1 and below.
			Database->{simple}->query("select procpid from pg_stat_activity")
		)->flat;
	}
...
sub tableIds ($) {
	...
	return ( Database->{simple}->select($table, 'id', {}, 'id')->flat );
}


sub lastTableId ($) {
	return [ tableIds(shift) ]->[-1] // 0;
}
