...
my %publication_search_columns = (
	...
	publication_advisory_website => [ qw(govcertid title) ],
);

my %publication_table_fields = (
	publication_advisory => "publication_advisory.title AS pub_title, '[v' || version || ']' AS version_str, xml_contents",
	publication_advisory_forward => "publication_advisory_forward.title AS pub_title, '[v' || version || ']' AS version_str, xml_contents",
	publication_advisory_website => "publication_advisory_website.title AS pub_title, '[v' || version || ']' AS version_str, xml_contents",
...
# getLatestAdvisoryVersion($advisoryID )
# Retrieve the latest version of an advisory. Returns the advisory as an HASH reference.
#     $obj->getLatestAdvisoryVersion('NCSC-2014-0001');
sub getLatestAdvisoryVersion($) {
	my ($self, $govcertId) = @_;
	my $db = Database->simple;

	my $table = $db->recordExists(publication_advisory_forward => { govcertid => $govcertId })
	  ? 'publication_advisory_forward'
	  : 'publication_advisory';

	$db->query(<<__LATEST_ADVISORY, $govcertId, $govcertId)->hash;
SELECT a.*
  FROM $table AS a
 WHERE NOT a.deleted
   AND a.govcertid = ?
   AND a.version IN (
       SELECT MAX(b.version)
         FROM $table AS b
        WHERE NOT b.deleted AND b.govcertid = ?
   )
__LATEST_ADVISORY
}
...
# processPreviewXml( $advisory )   # details
...
sub _to_CDATA($) {
	my $d = shift;
	  ref $d eq 'ARRAY' ? +[ map _to_CDATA($_), @$d ]
	: ref $d eq 'HASH'  ? +{ map +( $_ => _to_CDATA($d->{$_}) ), keys %$d }
	: $d =~ /\D/        ? '<![CDATA[' . decode_entities($d) . ']]>'
	:                     $d;
}
...
sub processPreviewXml {
	...
	my $advisory_id;
	if(reftype $publication eq 'HASH') {
		$advisory_id = $publication->{id};
	} else {
		$advisory_id = $publication;
		$publication = $self->getPublicationDetails( 
			table => "publication_advisory",
	...
	my $advisoryXMLTemplate = find_config $config->{advisory_xml_template};
	...
	my $organisation = $config->{organisation};
...
#	$publication->{issuer} = $usr->getUser( $publication->{created_by} )->{fullname};
	...
	# decoding is done because XMLout does not understand CDATA blocks;
	# encodes even de < of <![CDATA again :-(
	decode_entities $xml;
}
...
sub getDistinctPublicationTypes {
	...
	my $stmnt = "SELECT title FROM publication_type ORDER BY title DESC";
	$self->{dbh}->prepare($stmnt);
