...
# getPreviousAdvisoryId: retrieve the advisory ID which came before the given advisory ID.
# getPreviousAdvisory('FOO-2015-0002') => 'FOO-2015-0001'
# getPreviousAdvisory('FOO-2015-0001') => 'FOO-2014-0812' (last 2014 advisory)
sub getPreviousAdvisoryId {
	my ($self, $advisory_id, $table) = @_;
	$table //= 'publication_advisory';

	my ($advisoryPrefix, $year, $sequence_number) =
		$advisory_id =~ /^(.*)-(.*?)-(.*?)$/ or croak "invalid advisory id $advisory_id";

	if ($sequence_number == 1) {  # as in PREFIX-YEAR-0001
		# Get the last advisory id from last year.
		my $last_year = $year - 1;
		...
		return scalar Database->{simple}->select(
			...
			-where => {
				govcertid => {
					-ilike =>     "$advisoryPrefix-$last_year%",
					-not_ilike => "$advisoryPrefix-$last_year-X%",
				},
			},
			-order_by => {-desc =>
				# FOO-2015-0013 => 0013 => 13
				q{cast(regexp_replace(govcertid, '.*-', '') as integer)}
			},
			-limit => 1
		)->list;
	} else {
		return scalar Database->{simple}->select(
			...
			-where => {
				govcertid => {
					'~' => sprintf('^%s-%s-0*%d$', $advisoryPrefix, $year, $sequence_number - 1)
				}
			},
			-limit => 1
		)->list;
...
sub getPreviousAdvisoryForwardId {
	return getPreviousAdvisoryId(@_, 'publication_advisory_forward');
}
