...
sub new {
	...
	my $logDirectory = ( $config ) 
		? $config->{logdirectory} 
		: Taranis::Config->getSetting('logdirectory'); 
...
sub writeError {
	...
	if ( $content ) {
		my $logDirectory = $self->{logDirectory} || '/tmp';
...
=head1 SYNOPSIS
  ...
  $obj->writeError( digest => $sourceDigest, sourceName => $sourceName, error => 'Error message', error_code => '404', content => $errorContent );
...
Holds the logging directory setting which is set to C<logdirectory> setting in main configuration file taranis.conf.xml. 
Can be accessed by:
...
Method for deleting all database entries in table C<errors> and all files in log directory ( C<< $obj->{logDirectory} >> ).
...
=head2 writeError( digest => $sourceDigest, sourceName => $sourceName, error => 'Error message', error_code => '404', content => $errorContent )

Method for logging an error to database and creating a logfile. The contents of the logfile is set the C<content> argument. 

    $obj->writeError(
                       digest => 'UU/GiHB5oN0M+f7OMttUkA',
                       sourceName => 'source_x,
                       error => 'error message',
                       error_code => '010',
...
Caused by not deleteLog() , deleteLogs() or deleteAllLogs() when the log cannot be found in database. 
For deleteLog() you should check the C<$log_id> argument. For deletelogs() you should check the C<$errorCode> argument. 
...
You should check if the C<< $obj->{logDirectory} >> setting has been set correctly and if the rights for the directory are sufficient. 
You should also check if the supplied 'file_x' is present and if the rights for the file are sufficient for writing.
