...
#DBI->trace(0);
	#XXX working towards separate tabs for search categories: \%results
...
sub _searchAnyPublication($$%) {
	...
	$search->{scoring} = $self->_addSearchQuestion(
		...
		find_words   => [ publication => $details => 'contents' ],
...
sub _evaluateQuestion($) {
	...
	while($search_string =~ /\S/) {
		...
		if($is_tag) {
			$question{"has_tags"}++;
			push @{$question{"$need\_tags"}}, $word;
		} elsif ($self->_isIdentifier($word)) {
			# Identifiers are searched in special columns/tables, but also in texts
			$question{"has_certids"}++;
			push @{$question{"$need\_certids"}}, $word;
			$question{"has_words"}++;
			push @{$question{"$need\_words"}}, $word;
		} else {
			$question{"has_words"}++;
			push @{$question{"$need\_words"}}, $word;
		}
...
sub _addSearchQuestion($$$%) {
	...
	while(@pos_rules) {
		...
		if($class eq 'select_field') {
			...
			my $details = $table eq 'publication' ? shift @fields : $table;
	...
	if(@selects > 1 && ! $has_required) {
		...
		if(not grep !/^it\.digest IN /, @selects) {
			my @x = map s/^it\.digest IN //r, @selects;
			push @$where, 'it.digest = ANY(ARRAY(' . join(' UNION ', @x). '))'

		} elsif(not grep !/^details\.id IN /, @selects) {
			my @x = map s/^details\.id IN //r, @selects;
			push @$where, 'details.id = ANY(ARRAY(' . join(' UNION ', @x). '))'
...
sub _wordFilter($$$) {
	...
	if($table eq 'publication') {
		# Extra complication: the field is in the shared table, not in the
		# publication specific extension.  However, we do need the id of that
		# extension.
		my $details = $args->{details_table} or confess;
 ...
 SELECT d.id
   FROM $table AS pu
        JOIN $details AS d ON pu.id = d.publication_id
  WHERE $where
__SELECT
	}
