...
*getUserFullname      = \&Taranis::Users::Object::getUserFullname;
...
sub getMenuItems {
	...
	foreach my $program (keys %{ $entitlementsConfig->{entitlement} }) {
		my $alternative = $entitlementsConfig->{entitlement}->{$program};
		$alternative->{menuitem} or next;

		# Entitlements that give access to script $program.
		my $scriptEntitlements = $alternative->{use_entitlement};
		# Can be either a string or a ref to an array of strings; force into arrayref.
		ref $scriptEntitlements eq 'ARRAY'
			or $scriptEntitlements = [ $scriptEntitlements ];
		...
		foreach my $entitlement (keys %$rights) {
			my $access = $rights->{$entitlement};
			$access && $access->{read_right} or next;

			if($access->{particularization} eq '') {
				# String (entitlement name), e.g. 'generic'.
				$menuitems->{$program} = $entitlement;
			} else {
				# Arrayref, e.g. ['news'] or ['news', 'tweets'].
				$menuitems->{$program} = $access->{particularization};
			}
		}
	}
...
# Get only the user's fullname, even when the user is disabled.
sub getUserFullname($) {
	my ($self, $username) = @_;
...
sub addUser {
	...
	my %checkdata = ( username => $username );

	if ( $self->{dbh}->checkIfExists( \%checkdata, 'users' ) ) {
		$self->{errmsg} = "Username '$username' exists, perhaps the user is disabled?";
		return 0;
	}
