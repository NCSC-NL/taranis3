...
sub git_checkout($$) {
	...
	$has_version
		or die "ERROR: git sources not available: run 'taranis git init'\n";

	$has_version ne $need_version
		or return;

	if($need_version =~ /^([0-9]+\.[0-9]+)$/) {
		my $branch = "release-$1";

		print "*   checking out git branch '$branch'.\n";

		system "cd '$dir' && git checkout '$branch' 2>/dev/null"
			and die "ERROR: cannot not checkout branch '$branch'.\n";
	} else {

		my $tag = 'v'. $need_version;

		print "*   checking out git tag '$tag'.\n";

		system "cd '$dir' && git checkout '$tag' 2>/dev/null"
			and die "ERROR: could not checkout tag '$tag'.\n";
	}
