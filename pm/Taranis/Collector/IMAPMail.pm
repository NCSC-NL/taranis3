		...
		foreach my $msgSequenceNumber ( @messagesSequenceNumbers ) {
			...
			if (   ! $collector->itemExists($old_digest)
				...
				if ( $containsAdvisory ) {
					...
					if(my $attachments = $as->getAttachmentInfo($mimeEntity)) {
						my $validator;
...
sub importAdvisory {
	...
	my $collector = $self->{collector};
	my $an = Taranis::Analysis->new( $collector->{config} );
	...
	if ( $source->{create_advisory} && !$importError ) {
		my $config = $collector->{config};
		my $pu = Taranis::Publication->new($config);
		my $pa = Taranis::Publication::Advisory->new(config => $config);
		...
		my $typeId = $pu->getPublicationTypeId(advisory => 'email');
