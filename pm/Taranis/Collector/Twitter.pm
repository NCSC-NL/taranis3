...
sub getSourceData {
	...
	if ( $response->is_success ) {
		say $response->status_line if $debugSource;
		...
		my $error_line = $response->status_line;
		my $error_code = $response->code;

		my $error = "Could not retrieve url $retrieve_url $error_line";
		...
		$self->{http_status_line} = $error_line;
		$self->{http_status_code} = $error_code;
		return 0; 
	}
}
...
sub collect {
	...
	my ( @newTweets, $response );
 	...
 	my $tweets = ref $sourceData eq 'HASH' ? $sourceData->{statuses} : $sourceData;
