...
sub _downloadStatisticImages($$) {
  ...
  STAT:
	foreach my $stat (@$stats) {
		...
		my $blobDetails = $::db->{_Database}->addFileAsBlob(binary => $image);
		...
		my %update = (
			...
			source      => $source,
			category    => $category,
			object_id   => $blobDetails->{oid},
			file_size   => $blobDetails->{fileSize},
		);
...
sub _createGraph($%) {
	...
	my $blob   = $::db->{_Database}->addFileAsBlob(binary => $gd->gif);
	my %update = (
