...
sub deleteCollector {
	my ( $self, $collectorId ) = @_;
	
	if ( !$self->{dbh}->checkIfExists( { collector_id => $collectorId }, "sources") ) {

		my ( $stmnt, @bind ) = $self->{dbh}->{sql}->delete( 'collector', { id => $collectorId } );
		...
		$self->{errmsg} = 'Cannot delete collector, because collector is in use by one or more sources.';
		return 0;
	}
}
