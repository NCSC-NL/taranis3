...
sub connect {
	...
	my $simple   = $self->{simple} = DBIx::Simple->connect($dbh);

	# will disappear in T4.  In some cases, we currently need to access the
    # methods of Taranis::Database.
	$simple->{_Database} = $self;
	weaken $simple->{_Database};
...
sub checkIfExists {
	...
	return !!Database->{simple}->select($table, "count(*)", \%where)->list;
}
...
sub getResultCount {
	...
	return scalar Database->{simple}->query($stmnt, @bind)->list;
}
