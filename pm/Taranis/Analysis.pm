...
sub getRelatedAnalysisIdMatch {
	my ($self, $analysis_rights, $exclude_analysis_ids, @certids) = @_;
	@certids or return;

	my (@where, @binds);

	my @match_ids = map "idstring ILIKE '%$_%'", @certids;
	push @where, '('.join(' OR ', @match_ids).')';

	if(my $p = $analysis_rights->{particularization} ) {
		my @or = map "status ILIKE '$_'", @$p;
		push @where, '('.join(' OR ', @or).')';
	}

	if(my @exclude = flat $exclude_analysis_ids ) {
		my $exclude = join ',', @exclude;
		push @where, "id NOT IN ($exclude)";
	}

	my $where    = join "\n    AND ", @where;
	my @analyses = $self->{dbh}->simple->query(<<__MATCH, @binds)->hashes;
...
sub getRelatedAnalysisKeywordMatch {
	my ($self, $analysis_rights, $exclude_analysis_ids, @keywords) = @_;
	@keywords or return;

	my (@where, @binds);

	my @patterns = map <<__FIND_KEYWORD, map trim($_), @keywords;
SELECT id FROM analysis
 WHERE title ILIKE '%$_%' OR comments ILIKE '%$_%' OR idstring ILIKE '%$_%'
__FIND_KEYWORD

	my $wordfilter = join "\n        UNION ALL ", @patterns;

	my $db       = $self->{dbh}->simple;
	my @matches  = $db->query(<<__WORD_SEARCH)->flat;
SELECT id
  FROM ($wordfilter) AS matches
GROUP BY id
HAVING COUNT(*) > 1
__WORD_SEARCH

	my %exclude = map +($_ => 1), flat $exclude_analysis_ids;
	@matches    = grep ! $exclude{$_}, @matches;
	@matches or return [];

	# analysis-id column is varchar :-(
	my $matches  = join ',', map "'$_'", @matches;
	push @where, "id IN ($matches)";
	...
	my $where    = join "\n   AND ", @where;
	my @analyses = $db->query(<<__RECORDS, @binds)->hashes;
 SELECT id, title, status
   FROM analysis
  WHERE $where
  ORDER BY id DESC
...
sub getAnalysisRatingFromAdvisory {
	...
	#XXX analysisScale: low = 1, medium = 2, high = 3
