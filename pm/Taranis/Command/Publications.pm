...
sub publications_autosend_eos($) {
	...
	my $pubtempl_config = Config->{publication_templates};
	my $typeName  = Taranis::Config->new($pubtempl_config)->{eos}{email};
	my $eosTypeId = Publication->getPublicationTypeId( $typeName )->{id};
