...
SUB-COMMANDS:
  ...
  purge-unread -k|-r COUNT   reduce unread items to COUNT

OPTIONS:
  --before|-b DATE     DATE like 20170108 or +3d
  --schema|-s SCHEMA   Fix the schema version number
  --keep|-k COUNT      Keep newest COUNT unread items
  --read|-r COUNT      Read olders COUNT unread items
__HELP
} );
...
sub db_purge_unread($) {
	...
	my $assess = Taranis::Assess->new;
	my $unread = $assess->countItemStatus('unread');

	my $keep;
	if(my $read = $args->{read}) {
		$keep = $unread - $read;
		$keep = 0 if $keep < 0;
	} elsif($keep = $args->{keep}) {
	} else {
		die "ERROR: use either --keep or --read.\n";
	}

	print "Starting with $unread unread items.\n";
	...
	$assess->readOldestItems($keep);

	$unread = $assess->countItemStatus('unread');
	print "Ending-up with $unread unread items.\n";
}
