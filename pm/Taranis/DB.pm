...
use Taranis       qw(flat diff_lists);
...
# The required $guard parameter is only to explicitly relate the
...
=item my $h = $db->getRecord($table, $id, [$column]);

Returns the HASH from a C<$table>, for the unique key C<$id> in
C<$column> (which defaults to C<id>).
...
=item my @h = $db->getAllRecords($table, [@columns])

Returns a list of HASHes, each with one record in the C<$table>.  Only the
C<@columns> are returned, which defaults to all (C<*>).
...
sub getAllRecords($@) {
	...
	$self->query("SELECT $cols FROM $table")->hashes;
}


=item my $id = $db->addRecord($table, \%insert [, $column]);

=item (void) $db->addRecord($table, \%insert [, undef]);

A bit smarter than L<insert()>: the C<%insert> data gets added to the
C<$table>.

The content of the C<$column> (by default the C<id> field) gets returned.
When you are not interested in the C<id>-field, or when no unique
key is produced, then explicitly state 'undef' and/or use the method
in VOID content.
...
sub addRecord($$;$) {
	...
	return $self->insert($table, $insert)
		unless defined $retfield && defined wantarray;

	($self->insert($table, $insert, {returning => $retfield})->list)[0];
}
...
=item my $val = $db->recordExists($table, $id, [$key]);

=item my $val = $db->recordExists($table, \%where, [$key]);

Returns true if a record with a complex C<%where> query exists.  When only
a scalar C<$id> is passed, it will search in the column named 'id'.

When you specify a C<$key> (often 'id') you will get that value returned.
Otherwise, true is returned when the record is found.

=cut

sub recordExists($$$) {
	my ($self, $table, $where, $key) = @_;
	my $search = ref $where eq 'HASH' ? $where : { id => $where };
	(my $val) = $self->select($table, $key // 1, $search)->flat;
	$val;
}
...
=item $db->updateList($table, $field => \@values, %filter)
Manage a 'relation' table, which implement a one-to-many relation between
tables.  Returned are two arrays: the added values and the deleted values.

For instance, a constitution group may contain many individuals.
The C<$table> in this case maps group ids (in C<$field>) to individual
ids (as C<@values>) which should become the relation.

=cut

sub updateList($$$%) {
	my ($self, $table, $field, $want, %filter) = @_;
	defined $want or return;

	my $guard = $self->beginWork;

	my @has   = $self->select($table, $field, \%filter)->flat;
	my ($add, $delete) = diff_lists \@has, $want;

	$self->addRecord   ($table => { %filter, $field => $_ }) for @$add;
	$self->deleteRecord($table => { %filter, $field => $_ }) for @$delete;
...
=item $db->setList($table, $field => \@values, %other)
Create a new one-to-many relation between tables.  Rows are added to the table
which each C<$field> has a different value from C<@values> plus all C<%other>
details.

This is quite close to the C<updateList()> method, but then without any
attempt to delete existing records.

=cut

sub setList($$$%) {
	my ($self, $table, $field, $want, %other) = @_;
	defined $want or return;

	my $guard = $self->beginWork;
	$self->addRecord($table, { %other, $field => $_ }) for @$want;
	$self->commit($guard);
	$self;
}
