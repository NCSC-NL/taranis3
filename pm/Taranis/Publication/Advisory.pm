...
sub getLinksFromXMLAdvisory {
	...
	my @advisoryLinks = flat $xmlAdvisory->{content}->{additional_resources}->{resource};
	return join "\n", @advisoryLinks;
}
...
sub getSoftwareHardwareFromXMLAdvisory {
	...
	foreach my $shType ( 'product', 'platform' ) {
		my @xmlAdvisorySoftwareHardware = flat $xmlAdvisory->{meta_info}->{system_information}->{systemdetail}->{"affected_$shType"}->{$shType};
...
sub getDamageDescriptionsFromXMLAdvisory {
	...
	my @xmlDamageDescriptions = flat $xmlAdvisory->{meta_info}->{vulnerability_effect}->{effect};
