...
$EXPORT_TAGS{util} = [
	qw(
		...
		encode_entities_deep decode_entities_deep htmlToText textDigest normalizePath
		itemtype2text find_config tmp_path simplified_cpe scan_for_plugins
		shorten_html val_int flat trim_text
	)
];
$EXPORT_TAGS{all} = [
	@{$EXPORT_TAGS{util}},
	qw(
		keyword_ok decodeMimeEntity validateDateString sanitizeInput sanatizeLink
	)
];
...
# validateDateString($dateString): check if $dateString is formatted like 'dd-mm-yyyy' or 'd-m-yyyy'.
...
sub decodeMimeEntity {
	...
	if ( $entity->parts() ) {
		...
		} elsif ( $suggestedEncoding =~ /quoted-printable/i || $contentTransferEncoding =~ /quoted-printable/i ) {
...
# logDebug($text): only used for debugging/developing. Print to the debug logfile /tmp/Taranis_debug.log.
sub logDebug {
	my ($thing) = @_;

	my $thingString = ref $thing ? Data::Dumper->new([$thing])->Terse(1)->Dump : $thing;

	my $fh;
	my $path = tmp_path('debug.log');
	open($fh, ">>", $path) or croak "logDebug: failed to open $path for writing: $!";
	print $fh encode_utf8("[" . nowstring(7) . "]: $thingString\n");
	close $fh;
...
# Roughly strip some markup off of a chunk HTML (or XML) to get something that can (kind of) pass as plaintext.
# Note that this is NOT a replacement for proper HTML escaping; the resulting text may still contain HTML tags and
# should still be escaped when rendered as HTML.
sub htmlToText {
	my ( $filterthis ) = @_;

	# Collected HTML items are identified by, approximately, textDigest(htmlToText(<item-text-and-some-other-things>)).
	# This means that if these regexes are changed, the collector won't be able to recognize its own previously created
	# items, and will treat them all as new items. Tread carefully.
	$filterthis =~ s/\'//gi;
	$filterthis =~ s/\n/ /gi; # Ticket #167
	$filterthis =~ s/&lt;/</gi;
	$filterthis =~ s/&gt;/>/gi;
...
# T4 has many of these, used on thousands of places.  However, T3 is stable:
# only introduce this functions when the application gets serious testing.

sub val_int($) { defined $_[0] && $_[0] =~ /^\s*([0-9]+)\s*$/ ? $1 : undef }
