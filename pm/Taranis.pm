...
#XXX You cannot import Config, because that would overwrite the Taranis::Config
#XXX namespace with an object.
...
$EXPORT_TAGS{util} = [
	qw(
		...
		shorten_html val_int val_date val_string flat analysis_name trim_text
		diff_lists
	)
];
$EXPORT_TAGS{all} = [
...
sub decodeMimeEntity($$$) {
	...
	# $noHtmlPart excludes text/html parts.  So, together with $noAttachment is
	# intended to only return text/plain (which is an incorrect assumption)
	return " " if $noHtmlPart   && $contentType eq 'text/html';

	# $noAttachment avoids returning the content of non-text message parts.  Those
	# texts may be in attachments, so the option name is wrong.
	return " " if $noAttachment && $contentType !~ m!^text/!;
...
=item my $plugins = scan_for_plugins $subpkg, %options
...
The scan will get all modules which match C<pm/Taranis/$subpkg/*.pm> and
C<pm/$localns/$subpkg/*.pm> (where C<$localns> is the local package namespace
configured with C<local_package_base> in F<~/etc/taranis.conf.xml>.

Example:

  my $pkgs_hash = scan_for_plugins 'Command', load => 1;
  # May load  Taranis::Commmand::Install
  # and NCSC_NL::Command::Sage
...
sub scan_for_plugins($%) {
	my ($subspace, %args) = @_;
	my @pkgs = _scan_for_plugins "Taranis::$subspace", %args;

    if(my $local = Taranis::FunctionalWrapper::Config->getSetting('local_package_base')) {
        push @pkgs, _scan_for_plugins "${local}::$subspace", %args;
    }
...
=item my $string = val_string $value

Normalize a string input: blanks are removed from start and end, double
blanks removed.  An empty string will be returned as undef.
...
=item (\@add, \@del) = diff_lists \@old, \@new
Find the differences between two lists of items.
=cut
